...
bool initReceiver() {
  /*
      ...
      the time (since last reset) an event happens then runs an interrupt to
      ...
      -set timers' counter enable bit (turns on input capture mode)
      ...
      If it is we record the value in the channel's capture compare registor
      ...
      For more infomation on how to use input capture mode,
...
void sigChange1() {
  ...
  if (0b1 & GPIOA_BASE->IDR >> 0) {
    ...
    } //else if (ch1.pulseWidth > 4000) {
      //sometimes 
      //ch1.pulseWidth = big - ch1.pulseWidth
    // }
    TIMER2_BASE->CCER &= ~TIMER_CCER_CC1P;
  }
}
