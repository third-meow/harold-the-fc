...
void updateReciver(int *throttle, int *yaw, int *pitch, int *roll) {
  ...
  if (ch1.pulseWidth > 2000 || ch1.pulseWidth < 1000){
    ch.pulseWidth = ch1.lastPulseWidth;
  }
  
  *throttle = constrain(map(ch3.pulseWidth, 1000, 2000, 0, 255), 0, 255);
  
  *yaw = map(ch4.pulseWidth, 1000, 2000, -MAXTILT, MAXTILT);
  *pitch = map(ch2.pulseWidth, 1000, 2000, -MAXTILT, MAXTILT);
  *roll = map(ch1.pulseWidth, 1000, 2000, -MAXTILT, MAXTILT);
}
