...
void setup() {
  ...
  Serial3.println("initalizing receiver");
  initReceiver();

  if (!initIMU()) {
    Serial.println("NO BNO055 FOUND");
    Serial3.println("NO BNO055 FOUND");
  }

  //set all PID gains
  setGains();

  Serial3.println("Waiting for arming");
  ...
  //start motors
  Serial3.println("starting motors");
  initMotors();
...
void loop() {
  if (micros() - lastLoopStart >= loopGoal) {
    ...
    String outLog = "";
    outLog.concat(rateErrors.pitch[3]);
    outLog.concat("\t");
    outLog.concat(rateErrors.yaw[3]);
